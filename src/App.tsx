...
const CommandCenterPage = () => {
  ...
  return (
    <article className="command-page">
      ...
      <section className="command-performance" aria-labelledby="command-performance-heading">
        <div className="command-section__header">
          <div>
            <h2 id="command-performance-heading">{copy.performance.title}</h2>
            <p>{copy.performance.description}</p>
          </div>
          <p className="command-performance__note">{copy.performance.note}</p>
        </div>
        <div className="command-performance__grid">
          {copy.performance.metrics.map((metric, index) => (
            <article
              key={metric.id}
              className="command-performance__card"
              data-index={index}
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <header>
                <span className="command-performance__value">{metric.value}</span>
                <h3>{metric.label}</h3>
              </header>
              <p>{metric.detail}</p>
              <span className="command-performance__spark" aria-hidden="true">
                <GrowthSpark variant={index % 2 === 0 ? 'light' : 'dark'} size="sm" />
              </span>
            </article>
          ))}
      ...
      <section className="command-advisories" aria-labelledby="command-advisories-heading">
        <div className="command-section__header">
          <div>
            <h2 id="command-advisories-heading">{copy.advisories.title}</h2>
            <p>{copy.advisories.description}</p>
          </div>
        </div>
        <div className="command-advisories__grid">
          {copy.advisories.updates.map((update, index) => (
            <article
              key={update.id}
              className="command-advisories__card"
              data-index={index}
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <header className="command-advisories__header">
                <span className="command-advisories__issued">{update.issued}</span>
                <h3>{update.headline}</h3>
              </header>
              <p className="command-advisories__detail">{update.detail}</p>
              <footer className="command-advisories__footer">
                <span className="command-advisories__spark" aria-hidden="true">
                  <GrowthSpark variant={index % 2 === 0 ? 'light' : 'dark'} size="sm" />
                </span>
                <p>{update.response}</p>
              </footer>
      ...
      <section className="command-innovation" aria-labelledby="command-innovation-heading">
        <div className="command-section__header">
          <div>
            <h2 id="command-innovation-heading">{copy.innovation.title}</h2>
            <p>{copy.innovation.description}</p>
          </div>
        </div>
        <div className="command-innovation__grid">
          {copy.innovation.initiatives.map((initiative, index) => (
            <article
              key={initiative.id}
              className="command-innovation__card"
              data-index={index}
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <header className="command-innovation__header">
                <span className="command-innovation__badge" aria-hidden="true">
                  <GrowthSpark variant={index % 2 === 0 ? 'light' : 'dark'} size="sm" />
                </span>
                <h3>{initiative.name}</h3>
              </header>
              <p>{initiative.summary}</p>
              <footer className="command-innovation__meta">
                <span className="command-innovation__leader">{initiative.leader}</span>
                <span className="command-innovation__tempo">{initiative.tempo}</span>
              </footer>
            </article>
          ))}
        </div>
      ...
      <section className="command-labs" aria-labelledby="command-labs-heading">
        <div className="command-section__header">
          <div>
            <h2 id="command-labs-heading">{copy.labs.title}</h2>
            <p>{copy.labs.description}</p>
          </div>
        </div>
        <div className="command-labs__grid">
          {copy.labs.streams.map((stream, index) => (
            <article
              key={stream.id}
              className="command-labs__card"
              data-index={index}
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <span className="command-labs__spark" aria-hidden="true">
                <GrowthSpark variant={index % 2 === 0 ? 'dark' : 'light'} size="sm" />
              </span>
              <header className="command-labs__header">
                <span className="command-labs__badge" aria-hidden="true">{index + 1}</span>
                <div>
                  <h3>{stream.name}</h3>
                  <p className="command-labs__charter">{stream.charter}</p>
                </div>
              </header>
              <footer className="command-labs__footer">
                <span className="command-labs__owner">{stream.owner}</span>
                <span className="command-labs__signal">{stream.signal}</span>
              </footer>
            </article>
          ))}
        </div>
...
const ObservatoryPage = () => {
  ...
  return (
    <article className="observatory">
      ...
      <section
        className="observatory__section observatory__section--heatmaps"
        aria-labelledby="observatory-heatmaps-heading"
      >
        <div className="observatory__section-header">
          <div>
            <h2 id="observatory-heatmaps-heading">{copy.heatmaps.title}</h2>
            <p>{copy.heatmaps.description}</p>
          </div>
        </div>
        <div className="observatory__heatmap-grid">
          {copy.heatmaps.regions.map((region, index) => (
            <article
              key={region.id}
              className="observatory-card observatory-card--heatmap"
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <header>
                <h3>{region.region}</h3>
              ...
              <div className="observatory-card__metric">
                <span className="observatory-card__metric-value">{region.sentiment}</span>
                <span className="observatory-card__metric-delta">{region.movement}</span>
              </div>
              <dl className="observatory-card__meta">
                <div>
                  <dt>{labels.priority}</dt>
                  <dd>{region.priority}</dd>
                </div>
                <div>
                  <dt>{labels.action}</dt>
                  <dd>{region.action}</dd>
      ...
      <section
        className="observatory__section observatory__section--responders"
        aria-labelledby="observatory-responders-heading"
      >
        <div className="observatory__section-header">
          <div>
            <h2 id="observatory-responders-heading">{copy.responders.title}</h2>
            <p>{copy.responders.description}</p>
          </div>
        </div>
        <div className="observatory__responders-grid">
          {copy.responders.roster.map((responder, index) => (
            <article
              key={responder.id}
              className="observatory-card observatory-card--responder"
              style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
            >
              <header>
                <h3>{responder.name}</h3>
                <p className="observatory-card__role">{responder.role}</p>
              </header>
              <dl className="observatory-card__meta">
                <div>
                  <dt>{labels.coverage}</dt>
                  <dd>{responder.coverage}</dd>
                </div>
                <div>
                  <dt>{labels.specialties}</dt>
                  <dd>{responder.specialties}</dd>
                </div>
                <div>
                  <dt>{labels.languages}</dt>
                  <dd>{responder.languages}</dd>
                </div>
              </dl>
              <p className="observatory-card__note">{responder.background}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section
        className="observatory__section observatory__section--evidence"
        aria-labelledby="observatory-evidence-heading"
      >
        <div className="observatory__section-header">
          <div>
            <h2 id="observatory-evidence-heading">{copy.evidenceVault.title}</h2>
            <p>{copy.evidenceVault.description}</p>
          </div>
        </div>
        <div className="observatory__evidence-grid">
          {copy.evidenceVault.documents.map((document, index) => {
            const href = resolveObservatoryResourceHref(document.target, language)
            return (
              <article
                key={document.id}
                className="observatory-card observatory-card--evidence"
                style={{ animationDelay: `${index * 0.12}s` } as CSSProperties}
              >
                <header>
                  <h3>{document.name}</h3>
                </header>
                <p className="observatory-card__summary">{document.summary}</p>
                <dl className="observatory-card__meta">
                  <div>
                    <dt>{labels.type}</dt>
                    <dd>{document.type}</dd>
                  ...
                  <div>
                    <dt>{labels.classification}</dt>
                    <dd>{document.classification}</dd>
                  </div>
                </dl>
                <Link className="button ghost observatory-card__action" to={href}>
                  {document.cta}
